      ...
      _export("Gun", Gun = (_dec = ccclass('Gun'), _dec2 = property(Prefab), _dec3 = property(Node), _dec(_class = (_class2 = class Gun extends Component {
        ...
        update(dt) {
          ...
          if (this.isShoot) {
            ...
            let bullet = (_crd && Bullet === void 0 ? (_reportPossibleCrUseOfBullet({
              error: Error()
            }), Bullet) : Bullet).get(this.bullet);
            bullet == null || bullet.insert(parent); //添加到显示父节点

            bullet == null || bullet.init(); //parent.getComponent(UITransform).convertToNodeSpaceAR(this.point.worldPosition,tempPos);
            //枪管挂点世界坐标和子弹父亲节点世界坐标的偏移坐标，就是实际当前的子坐标，其实和上面转换是等价的

            Vec3.subtract(tempPos, this.point.worldPosition, parent.worldPosition);
            bullet == null || bullet.setPosition(tempPos); //计算发射的起点
            //计算发射角度

            let dir = this.direction.normalize();
            let angle = Math.atan2(dir.y, dir.x);
            Quat.rotateZ(tempRot, Quat.IDENTITY, angle);
            bullet == null || bullet.setRotation(tempRot); //发射速度和生命时长

            bullet == null || bullet.velocity.set(dir).multiplyScalar(this.speed);
...
//# sourceMappingURL=f4e7c84ab072fc4dc4d0242e0da3b957dbaf04e8.js.map